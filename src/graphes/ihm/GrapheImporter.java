...
import graphes.IPCC;
import graphes.algorithmes.PCCDijkstra;
...
public class GrapheImporter {
	public static void main(String[] args) throws NumberFormatException, FileNotFoundException, IOException {
		...
		System.out.println(pcc.plusCourtChemin(g, df.getSource(), df.getDestination(), chemin));
	...
	/**
	 * Affiche le resultat sous le format attendu
	 * @param chemin
	 * @return
	 */
	...
	/**
	 * Affiche le resultat obtenu avec Dijktra, exercice 3.1
	 */
	static void afficherExo31() {
		...
		// il aurait ete plus pratique d'ailleurs de la mettre dans un fichier texte
		...
		int distance = pcc.plusCourtChemin(g, 1, 6, chemin);
	...
	public static boolean comparer(String fichierGraphe, String fichierReponse, IPCC algo) throws NumberFormatException, IOException {
		...
		try {
			int distanceCalculee = algo.plusCourtChemin(g, df.getSource(), df.getDestination(), cheminCalcule);
			...
			System.out.println(fichierGraphe + " vs " +  fichierReponse);
			System.out.println("Message attendu : " + ("pas de chemin entre " + df.getSource() + " et " + df.getDestination()));
			System.out.println("Message fait : " + e.getMessage());
			System.out.println("========================================"); // saut de ligne
			return ("pas de chemin entre " + df.getSource() + " et " + df.getDestination()).equals(e.getMessage());
		}
	}
...
public static void verifierGraphes() throws FileNotFoundException {
	...
	File dir = new File(dirStr);
	  ...
	  if (directoryListing != null) {
	    for (File child : directoryListing) {
	      ...
	      System.out.println("debut et fin du chemin a trouver : "+ df.getSource()+ " ==> "+ df.getDestination()+"\n");
	    }
	  } else {
	    System.out.println("Erreur : "+ dirStr + " n'est pas un reperoire");
	  }
}
	
	/**
	 * Retourne l'arc correctement initialise
	 * @param string
	 * @return
	 */
	...
	public static int importerReponse(String filePath, List<Integer> chemin) throws FileNotFoundException {
		...
		line = sc.nextLine(); // nom de l'algo recommand�
